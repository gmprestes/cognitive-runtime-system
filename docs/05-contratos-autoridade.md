...
Se o modelo pode mutar sozinho o que a organização acredita e reutiliza, você não tem runtime cognitivo — tem monólogo com side effects.
...
| **Host** | executar ações no mundo; devolver observations | escrever cold memory fora das APIs do runtime |
| **Human** | review de memória; override mediado; halt | bypass de audit log (SHOULD NOT) |

## Contrato III — Proposal

Todo output de modelo que possa afetar cognição/orquestração **deve** vir envelopado:
...
| `propose_belief` | crença candidata |
| `propose_lesson` | lição candidata |
| `request_human_review` | escalada |
| `halt` | parar com motivo |

Openings desconhecidas → **reject**.

## Contrato IV — Aprendizagem
...
- max wall-clock  
- max cost  
- max memory writes candidatas  

Esgotou → fase `stop` com razão.  
Sem budget, o sistema não é CRS-conforme.

## Lei curta (para gravar)
